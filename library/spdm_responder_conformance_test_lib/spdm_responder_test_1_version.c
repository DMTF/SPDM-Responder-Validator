...
void spdm_test_case_version_success (void *test_context)
{
    ...
    if ((spdm_response->version_number_entry_count > 0) &&
        (spdm_response->version_number_entry_count <= (spdm_response_size - sizeof(spdm_version_response_t)) / sizeof(spdm_version_number_t))) {
        test_result = COMMON_TEST_RESULT_PASS;
    } else {
        test_result = COMMON_TEST_RESULT_FAIL;
    }
    common_test_record_test_assertion (
        ...
        test_result, "response version_number_entry_count - 0x%02x", spdm_response->version_number_entry_count);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    for (index = 0; index < spdm_response->version_number_entry_count; index++) {
        ...
        if (version == SPDM_MESSAGE_VERSION_10 || version == SPDM_MESSAGE_VERSION_11 || version == SPDM_MESSAGE_VERSION_12) {
            test_result = COMMON_TEST_RESULT_PASS;
        } else {
            test_result = COMMON_TEST_RESULT_FAIL;
        }
...
common_test_case_t m_spdm_test_group_version[] = {
    {SPDM_RESPONDER_TEST_CASE_VERSION_SUCCESS_10,         "spdm_test_case_version_success", spdm_test_case_version_success},
    {SPDM_RESPONDER_TEST_CASE_VERSION_INVALID_REQUEST, "spdm_test_case_version_invalid_request", spdm_test_case_version_invalid_request},
