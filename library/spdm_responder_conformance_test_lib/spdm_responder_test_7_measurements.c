    ...
    size_t spdm_version_count, spdm_version_number_t *spdm_version)
{
    ...
    if (need_session) {
        ...
        status = libspdm_start_session (spdm_context, false, SPDM_KEY_EXCHANGE_REQUEST_ALL_MEASUREMENTS_HASH,
                                              ...
                                              uint8_t *measurement_index_mask)
{
    ...
    for (index = 0; index < number_of_blocks_in; index++) {
        ...
        if (spdm_version < SPDM_MESSAGE_VERSION_12) {
            ...
            measurment_data_size += sizeof(spdm_measurement_block_common_header_t) + measurement_size;
        }
    ...
    uint32_t *measurement_record_length_out)
{
    ...
    *measurement_record_length_out = 0;
    ...
    for (index = 0; index < number_of_blocks; index++) {
        LIBSPDM_ASSERT ((size_t)measurement_record_in + sizeof(spdm_measurement_block_common_header_t) <
            (size_t)measurement_record_end);
        measurement_size = ((spdm_measurement_block_common_header_t *)measurement_record_in)->measurement_size;
        LIBSPDM_ASSERT ((size_t)measurement_record_in + sizeof(spdm_measurement_block_common_header_t) + measurement_size <=
            (size_t)measurement_record_end);

        if (measurement_index == ((spdm_measurement_block_common_header_t *)measurement_record_in)->index) {
        ...
        measurement_record_in = (void *)((size_t)measurement_record_in + sizeof(spdm_measurement_block_common_header_t) + measurement_size);
    }
...
void spdm_test_case_measurements_success_10_11_12 (void *test_context, uint8_t version, bool need_session)
{
    spdm_test_context_t *spdm_test_context;
    void *spdm_context;
    libspdm_return_t status;
    ...
    for (slot_id = 0; slot_id < SPDM_MAX_SLOT_COUNT; slot_id++) {
        ...
        if ((test_buffer->rsp_cap_flags & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MEAS_CAP_SIG) != 0) {
            ...
            if (version >= SPDM_MESSAGE_VERSION_10) {
                if ((spdm_response->header.param2 & SPDM_MEASUREMENTS_RESPONSE_SLOT_ID_MASK) == slot_id) {
                    test_result = COMMON_TEST_RESULT_PASS;
                } else {
                    test_result = COMMON_TEST_RESULT_FAIL;
                }
...
void spdm_test_case_measurements_unexpected_request_in_session (void *test_context)
{
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_MEASUREMENTS, SPDM_RESPONDER_TEST_CASE_MEASUREMENTS_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 1,
        test_result, "response size - %d", spdm_response_size);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_MEASUREMENTS, SPDM_RESPONDER_TEST_CASE_MEASUREMENTS_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 2,
        test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_MEASUREMENTS, SPDM_RESPONDER_TEST_CASE_MEASUREMENTS_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 3,
        test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_MEASUREMENTS, SPDM_RESPONDER_TEST_CASE_MEASUREMENTS_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 4,
        test_result, "response param1 - 0x%02x", spdm_response->header.param1);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_MEASUREMENTS, SPDM_RESPONDER_TEST_CASE_MEASUREMENTS_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 5,
        test_result, "response param2 - 0x%02x", spdm_response->header.param2);
}
...
common_test_case_t m_spdm_test_group_measurements[] = {
    ...
    {SPDM_RESPONDER_TEST_CASE_MEASUREMENTS_SUCCESS_11_IN_DHE_SESSION, "spdm_test_case_measurements_success_11_session", spdm_test_case_measurements_success_11_session, spdm_test_case_measurements_setup_version_11_session},
    {SPDM_RESPONDER_TEST_CASE_MEASUREMENTS_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, "spdm_test_case_measurements_unexpected_request_in_session", spdm_test_case_measurements_unexpected_request_in_session, spdm_test_case_measurements_setup_version_any_session_cap},
    ...
    {SPDM_RESPONDER_TEST_CASE_MEASUREMENTS_SUCCESS_12_IN_DHE_SESSION, "spdm_test_case_measurements_success_12_session", spdm_test_case_measurements_success_12_session, spdm_test_case_measurements_setup_version_12_session},
