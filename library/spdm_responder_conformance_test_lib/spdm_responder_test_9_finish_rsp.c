...
/* Builds a FINISH request for negotiated version 1.1~1.4 and returns the
 * offset of RequesterVerifyData in *verify_data_ptr. Version 1.4+ inserts a
 * 2-byte OpaqueDataLength field (always 0 here) between the fixed header
 * and RequesterVerifyData. */
static size_t spdm_test_case_finish_rsp_build_request (uint8_t version,
                                                       ...
                                                       uint8_t **verify_data_ptr)
{
    ...
    *verify_data_ptr = ptr;
    ...
    return (size_t)(ptr - (uint8_t *)spdm_request);
}
...
{
    return spdm_test_case_finish_rsp_setup_vca_digest (test_context, 0, NULL, false, false);
}

bool spdm_test_case_finish_rsp_setup_version_any_hs_clear (void *test_context)
{
    return spdm_test_case_finish_rsp_setup_vca_digest (test_context, 0, NULL, true, false);
}
...
bool spdm_test_case_finish_rsp_setup_version_13_multi_key (void *test_context)
{
    spdm_version_number_t spdm_version[] = {
        SPDM_MESSAGE_VERSION_13 << SPDM_VERSION_NUMBER_SHIFT_BIT,
        SPDM_MESSAGE_VERSION_14 << SPDM_VERSION_NUMBER_SHIFT_BIT
...
bool spdm_test_case_finish_rsp_setup_version_13_hs_clear_multi_key (void *test_context)
{
    spdm_version_number_t spdm_version[] = {
        SPDM_MESSAGE_VERSION_13 << SPDM_VERSION_NUMBER_SHIFT_BIT,
        SPDM_MESSAGE_VERSION_14 << SPDM_VERSION_NUMBER_SHIFT_BIT
                                              ...
                                              bool multi_key)
{
    ...
    case SPDM_MESSAGE_VERSION_14:
        ...
        if (multi_key) {
            if (hs_clear) {
                case_id = SPDM_RESPONDER_TEST_CASE_FINISH_RSP_SUCCESS_13_HS_CLEAR_MULTI_KEY;
            } else {
                case_id = SPDM_RESPONDER_TEST_CASE_FINISH_RSP_SUCCESS_13_MULTI_KEY;
            }
        } else {
            if (hs_clear) {
    ...
    for (slot_id = 0; slot_id < SPDM_MAX_SLOT_COUNT; slot_id++) {
        ...
        spdm_request_size = spdm_test_case_finish_rsp_build_request (
            test_buffer->version, &spdm_request, &ptr);
        ...
        spdm_request_size = spdm_request_size + test_buffer->hash_size;
        ...
        opaque_length_ptr = NULL;
        if (test_buffer->version >= SPDM_MESSAGE_VERSION_14) {
            if (spdm_response_size < sizeof(spdm_finish_response_t) +
                sizeof(uint16_t) + verify_data_size) {
                test_result = COMMON_TEST_RESULT_FAIL;
            } else {
                opaque_length_ptr =
                    (void *)((size_t)spdm_response + sizeof(spdm_finish_response_t));
                if (spdm_response_size < sizeof(spdm_finish_response_t) +
                    sizeof(uint16_t) + *opaque_length_ptr + verify_data_size) {
                    test_result = COMMON_TEST_RESULT_FAIL;
                } else {
        ...
        opaque_entry_size = (opaque_length_ptr != NULL) ?
                            sizeof(uint16_t) + *opaque_length_ptr : 0;
        ...
        status = libspdm_append_message_f(spdm_context, session_info, true, spdm_response,
                                          sizeof(spdm_finish_response_t) + opaque_entry_size);
        if (LIBSPDM_STATUS_IS_ERROR(status)) {
            common_test_record_test_assertion (
                SPDM_RESPONDER_TEST_GROUP_FINISH_RSP, case_id, 0,
...
void spdm_test_case_finish_rsp_success_13_multi_key (void *test_context)
{
    spdm_test_case_finish_rsp_success_11_12 (test_context,
                                             SPDM_MESSAGE_VERSION_13, false, true);
}

void spdm_test_case_finish_rsp_success_13_hs_clear_multi_key (void *test_context)
{
    spdm_test_case_finish_rsp_success_11_12 (test_context,
...
common_test_case_t m_spdm_test_group_finish_rsp[] = {
    ...
    {SPDM_RESPONDER_TEST_CASE_FINISH_RSP_SESSION_REQUIRED,
     ...
     spdm_test_case_finish_rsp_setup_version_12,
     spdm_test_case_common_teardown},
    {SPDM_RESPONDER_TEST_CASE_FINISH_RSP_SUCCESS_13_MULTI_KEY,
     "spdm_test_case_finish_rsp_success_13_multi_key",
     spdm_test_case_finish_rsp_success_13_multi_key,
     spdm_test_case_finish_rsp_setup_version_13_multi_key,
     spdm_test_case_common_teardown},
    {SPDM_RESPONDER_TEST_CASE_FINISH_RSP_SUCCESS_13_HS_CLEAR_MULTI_KEY,
     "spdm_test_case_finish_rsp_success_13_hs_clear_multi_key",
     spdm_test_case_finish_rsp_success_13_hs_clear_multi_key,
     spdm_test_case_finish_rsp_setup_version_13_hs_clear_multi_key,
     spdm_test_case_common_teardown},
