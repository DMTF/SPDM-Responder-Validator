...
bool spdm_test_case_challenge_auth_setup_vca_digest (void *test_context,
    size_t spdm_version_count, spdm_version_number_t *spdm_version)
{
    ...
    LIBSPDM_ASSERT(sizeof(spdm_test_context->test_scratch_buffer) >= sizeof(spdm_challenge_auth_test_buffer_t));
...
bool spdm_test_case_challenge_auth_setup_version_10_11 (void *test_context)
{
    ...
    return spdm_test_case_challenge_auth_setup_vca_digest (test_context,
        LIBSPDM_ARRAY_SIZE(spdm_version), spdm_version);
}
...
void spdm_test_case_challenge_auth_success_10_12 (void *test_context, uint8_t version, uint8_t message_mask)
{
    spdm_test_context_t *spdm_test_context;
    void *spdm_context;
    libspdm_return_t status;
    ...
    case SPDM_MESSAGE_VERSION_10:
        ...
        switch (message_mask) {
        case SPDM_MESSAGE_A_MASK_VCA | SPDM_MESSAGE_B_MASK_GET_DIGESTS | SPDM_MESSAGE_B_MASK_GET_CERTIFICATE:
    ...
    for (slot_id = 0; slot_id < SPDM_MAX_SLOT_COUNT; slot_id++) {
        ...
        for (meas_hash_type_index = 0; meas_hash_type_index < LIBSPDM_ARRAY_SIZE(measurement_hash_type); meas_hash_type_index++) {
            if ((test_buffer->rsp_cap_flags & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MEAS_CAP) == 0) {
                if (measurement_hash_type[meas_hash_type_index] != SPDM_CHALLENGE_REQUEST_NO_MEASUREMENT_SUMMARY_HASH) {
                    continue;
                }
            }
            ...
            if ((message_mask & SPDM_MESSAGE_A_MASK_VCA) == 0) {
                status = libspdm_challenge (spdm_context, slot_id, measurement_hash_type[meas_hash_type_index], NULL, NULL);
                if (LIBSPDM_STATUS_IS_ERROR(status)) {
                    common_test_record_test_assertion (
                        SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, case_id, COMMON_TEST_ID_END,
            ...
            opaque_length_ptr = (void *)((size_t)spdm_response + sizeof(spdm_challenge_auth_response_t) +
            ...
            cert_chain_hash_ptr = (void *)((size_t)spdm_response + sizeof(spdm_challenge_auth_response_t));
            signature_ptr = (void *)((size_t)spdm_response + sizeof(spdm_challenge_auth_response_t) +
                                        test_buffer->hash_size + SPDM_NONCE_SIZE +
                                        meas_hash_size + sizeof(uint16_t) +
                                        *opaque_length_ptr);
            ...
            if ((spdm_response->header.param1 & SPDM_CHALLENGE_AUTH_RESPONSE_ATTRIBUTE_SLOT_ID_MASK) == slot_id) {
                test_result = COMMON_TEST_RESULT_PASS;
            } else {
                test_result = COMMON_TEST_RESULT_FAIL;
            }
...
void spdm_test_case_challenge_auth_success_10_a1b1c1 (void *test_context)
{
    spdm_test_case_challenge_auth_success_10_12 (test_context,
        ...
        SPDM_MESSAGE_B_MASK_GET_DIGESTS | SPDM_MESSAGE_B_MASK_GET_CERTIFICATE);
}
...
void spdm_test_case_challenge_auth_version_mismatch (void *test_context)
{
    ...
    for (index = 0; index < LIBSPDM_ARRAY_SIZE(mismatched_version); index++) {
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_VERSION_MISMATCH, 1,
            test_result, "response size - %d", spdm_response_size);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_VERSION_MISMATCH, 2,
            test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_VERSION_MISMATCH, 3,
            test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_VERSION_MISMATCH, 4,
            test_result, "response param1 - 0x%02x", spdm_response->header.param1);
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_VERSION_MISMATCH, 5,
            test_result, "response param2 - 0x%02x", spdm_response->header.param2);
    }
}
...
void spdm_test_case_challenge_auth_unexpected_request (void *test_context)
{
    ...
    if (LIBSPDM_STATUS_IS_ERROR(status)) {
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_UNEXPECTED_REQUEST, COMMON_TEST_ID_END,
            COMMON_TEST_RESULT_NOT_TESTED, "send/receive failure");
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_UNEXPECTED_REQUEST, 1,
        test_result, "response size - %d", spdm_response_size);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_UNEXPECTED_REQUEST, 2,
        test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_UNEXPECTED_REQUEST, 3,
        test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_UNEXPECTED_REQUEST, 4,
        test_result, "response param1 - 0x%02x", spdm_response->header.param1);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_UNEXPECTED_REQUEST, 5,
        test_result, "response param2 - 0x%02x", spdm_response->header.param2);
}
    ...
    for (index = 0; index < SPDM_MAX_SLOT_COUNT * 2 + 2; index++) {
        ...
        if (index < SPDM_MAX_SLOT_COUNT * 2) {
            ...
            common_test_record_test_message ("test invalid meas_hash_type - 0x%02x\n",
                SPDM_CHALLENGE_REQUEST_TCB_COMPONENT_MEASUREMENT_HASH + 1);
            spdm_request_new.header.param2 = SPDM_CHALLENGE_REQUEST_TCB_COMPONENT_MEASUREMENT_HASH + 1;
        ...
        if (LIBSPDM_STATUS_IS_ERROR(status)) {
            common_test_record_test_assertion (
                SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_INVALID_REQUEST, COMMON_TEST_ID_END,
                COMMON_TEST_RESULT_NOT_TESTED, "send/receive failure");
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_INVALID_REQUEST, 1,
            test_result, "response size - %d", spdm_response_size);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_INVALID_REQUEST, 2,
            test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_INVALID_REQUEST, 3,
            test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_INVALID_REQUEST, 4,
            test_result, "response param1 - 0x%02x", spdm_response->header.param1);
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_CHALLENGE_AUTH, SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_INVALID_REQUEST, 5,
            test_result, "response param2 - 0x%02x", spdm_response->header.param2);
    }
}

common_test_case_t m_spdm_test_group_challenge_auth[] = {
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_10_A1B1C1, "spdm_test_case_challenge_auth_success_10_a1b1c1", spdm_test_case_challenge_auth_success_10_a1b1c1, spdm_test_case_challenge_auth_setup_version_10_11},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_10_A1B2C1, "spdm_test_case_challenge_auth_success_10_a1b2c1", spdm_test_case_challenge_auth_success_10_a1b2c1, spdm_test_case_challenge_auth_setup_version_10_11},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_10_A1B3C1, "spdm_test_case_challenge_auth_success_10_a1b3c1", spdm_test_case_challenge_auth_success_10_a1b3c1, spdm_test_case_challenge_auth_setup_version_10_11},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_VERSION_MISMATCH, "spdm_test_case_challenge_auth_version_mismatch", spdm_test_case_challenge_auth_version_mismatch, spdm_test_case_challenge_auth_setup_version_any},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_UNEXPECTED_REQUEST, "spdm_test_case_challenge_auth_unexpected_request", spdm_test_case_challenge_auth_unexpected_request, spdm_test_case_challenge_auth_setup_version_capabilities},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_INVALID_REQUEST, "spdm_test_case_challenge_auth_invalid_request", spdm_test_case_challenge_auth_invalid_request, spdm_test_case_challenge_auth_setup_version_any},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_12_A1B1C1, "spdm_test_case_challenge_auth_success_12_a1b1c1", spdm_test_case_challenge_auth_success_12_a1b1c1, spdm_test_case_challenge_auth_setup_version_12},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_12_A1B2C1, "spdm_test_case_challenge_auth_success_12_a1b2c1", spdm_test_case_challenge_auth_success_12_a1b2c1, spdm_test_case_challenge_auth_setup_version_12},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_12_A1B3C1, "spdm_test_case_challenge_auth_success_12_a1b3c1", spdm_test_case_challenge_auth_success_12_a1b3c1, spdm_test_case_challenge_auth_setup_version_12},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_12_A1B4C1, "spdm_test_case_challenge_auth_success_12_a1b4c1", spdm_test_case_challenge_auth_success_12_a1b4c1, spdm_test_case_challenge_auth_setup_version_12},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_12_A2B1C1, "spdm_test_case_challenge_auth_success_12_a2b1c1", spdm_test_case_challenge_auth_success_12_a2b1c1, spdm_test_case_challenge_auth_setup_version_12},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_12_A2B2C1, "spdm_test_case_challenge_auth_success_12_a2b2c1", spdm_test_case_challenge_auth_success_12_a2b2c1, spdm_test_case_challenge_auth_setup_version_12},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_12_A2B3C1, "spdm_test_case_challenge_auth_success_12_a2b3c1", spdm_test_case_challenge_auth_success_12_a2b3c1, spdm_test_case_challenge_auth_setup_version_12},
    {SPDM_RESPONDER_TEST_CASE_CHALLENGE_AUTH_SUCCESS_12_A2B4C1, "spdm_test_case_challenge_auth_success_12_a2b4c1", spdm_test_case_challenge_auth_success_12_a2b4c1, spdm_test_case_challenge_auth_setup_version_12},
