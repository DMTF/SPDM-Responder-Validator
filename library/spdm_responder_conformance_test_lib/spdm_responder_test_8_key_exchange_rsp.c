    ...
    size_t spdm_version_count, spdm_version_number_t *spdm_version, bool hs_clear)
{
    ...
    LIBSPDM_ASSERT(sizeof(spdm_test_context->test_scratch_buffer) >= sizeof(spdm_key_exchange_rsp_test_buffer_t));
    ...
    libspdm_get_data(spdm_context, LIBSPDM_DATA_DHE_NAME_GROUP, &parameter, &test_buffer->dhe_named_group, &data_size);
    ...
    spdm_test_context->test_scratch_buffer_size = offsetof(spdm_key_exchange_rsp_test_buffer_t, total_digest_buffer) +
...
void spdm_test_case_key_exchange_rsp_success_11_12 (void *test_context, uint8_t version, bool hs_clear)
{
    spdm_test_context_t *spdm_test_context;
    void *spdm_context;
    libspdm_return_t status;
    ...
    for (slot_id = 0; slot_id < SPDM_MAX_SLOT_COUNT; slot_id++) {
        ...
        for (meas_hash_type_index = 0; meas_hash_type_index < LIBSPDM_ARRAY_SIZE(measurement_hash_type); meas_hash_type_index++) {
            if ((test_buffer->rsp_cap_flags & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MEAS_CAP) == 0) {
                if (measurement_hash_type[meas_hash_type_index] != SPDM_KEY_EXCHANGE_REQUEST_NO_MEASUREMENT_SUMMARY_HASH) {
                    continue;
                }
            }
            ...
            *(uint16_t *)ptr = (uint16_t)opaque_key_exchange_req_size;
            ...
            opaque_length_ptr = (void *)((size_t)spdm_response + sizeof(spdm_key_exchange_response_t) +
            ...
            exchange_data_ptr = (void *)((size_t)spdm_response + sizeof(spdm_key_exchange_response_t));
            ...
            common_test_record_test_assertion (
                ...
                test_result, "response mut_auth_requested - 0x%02x", spdm_response->mut_auth_requested);
            if (test_result == COMMON_TEST_RESULT_FAIL) {
                libspdm_secured_message_dhe_free(
                    test_buffer->dhe_named_group, dhe_context);
                return ;
            ...
            common_test_record_test_assertion (
                SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, case_id, 4,
                test_result, "response req_slot_id_param - 0x%02x", spdm_response->req_slot_id_param);
            if (test_result == COMMON_TEST_RESULT_FAIL) {
                libspdm_secured_message_dhe_free(
                    test_buffer->dhe_named_group, dhe_context);
                return ;
            ...
            secured_message_context = libspdm_get_secured_message_context_via_session_info(session_info);
...
void spdm_test_case_key_exchange_rsp_version_mismatch (void *test_context)
{
    ...
    for (index = 0; index < LIBSPDM_ARRAY_SIZE(mismatched_version); index++) {
        ...
        *(uint16_t *)ptr = (uint16_t)opaque_key_exchange_req_size;
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_VERSION_MISMATCH, 1,
            test_result, "response size - %d", spdm_response_size);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_VERSION_MISMATCH, 2,
            test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_VERSION_MISMATCH, 3,
            test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_VERSION_MISMATCH, 4,
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_VERSION_MISMATCH, 5,
...
void spdm_test_case_key_exchange_rsp_unexpected_request (void *test_context)
{
    ...
    *(uint16_t *)ptr = 0;
    ...
    if (LIBSPDM_STATUS_IS_ERROR(status)) {
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST, COMMON_TEST_ID_END,
            COMMON_TEST_RESULT_NOT_TESTED, "send/receive failure");
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST, 1,
        test_result, "response size - %d", spdm_response_size);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST, 2,
        test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST, 3,
        test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST, 4,
        test_result, "response param1 - 0x%02x", spdm_response->header.param1);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST, 5,
        test_result, "response param2 - 0x%02x", spdm_response->header.param2);
}
...
void spdm_test_case_key_exchange_rsp_unexpected_request_in_session (void *test_context)
{
    ...
    if (LIBSPDM_STATUS_IS_ERROR(status)) {
        common_test_record_test_assertion (
                    SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST_IN_SESSION, COMMON_TEST_ID_END,
    ...
    *(uint16_t *)ptr = (uint16_t)opaque_key_exchange_req_size;
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST_IN_SESSION, 1,
        test_result, "response size - %d", spdm_response_size);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST_IN_SESSION, 2,
        test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST_IN_SESSION, 3,
        test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST_IN_SESSION, 4,
        test_result, "response param1 - 0x%02x", spdm_response->header.param1);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return ;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST_IN_SESSION, 5,
        test_result, "response param2 - 0x%02x", spdm_response->header.param2);
}
    ...
    *(uint16_t *)ptr = (uint16_t)opaque_key_exchange_req_size;
    ...
    for (index = 0; index < SPDM_MAX_SLOT_COUNT * 2 + 2; index++) {
        ...
        if (index < SPDM_MAX_SLOT_COUNT * 2) {
            ...
            spdm_request_new.header.param1 = SPDM_KEY_EXCHANGE_REQUEST_TCB_COMPONENT_MEASUREMENT_HASH + 1;
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_INVALID_REQUEST, 1,
            test_result, "response size - %d", spdm_response_size);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_INVALID_REQUEST, 2,
            test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_INVALID_REQUEST, 3,
            test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue ;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_INVALID_REQUEST, 4,
            test_result, "response param1 - 0x%02x", spdm_response->header.param1);
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_KEY_EXCHANGE_RSP, SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_INVALID_REQUEST, 5,
            test_result, "response param2 - 0x%02x", spdm_response->header.param2);
    }
}

common_test_case_t m_spdm_test_group_key_exchange_rsp[] = {
    {SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_SUCCESS_11, "spdm_test_case_key_exchange_rsp_success_11", spdm_test_case_key_exchange_rsp_success_11, spdm_test_case_key_exchange_rsp_setup_version_11},
    {SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_SUCCESS_11_HS_CLEAR, "spdm_test_case_key_exchange_rsp_success_11_hs_clear", spdm_test_case_key_exchange_rsp_success_11_hs_clear, spdm_test_case_key_exchange_rsp_setup_version_11_hs_clear},
    {SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_VERSION_MISMATCH, "spdm_test_case_key_exchange_rsp_version_mismatch", spdm_test_case_key_exchange_rsp_version_mismatch, spdm_test_case_key_exchange_rsp_setup_version_any},
    {SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST, "spdm_test_case_key_exchange_rsp_unexpected_request", spdm_test_case_key_exchange_rsp_unexpected_request, spdm_test_case_key_exchange_rsp_setup_version_capabilities},
    {SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_UNEXPECTED_REQUEST_IN_SESSION, "spdm_test_case_key_exchange_rsp_unexpected_request_in_session", spdm_test_case_key_exchange_rsp_unexpected_request_in_session, spdm_test_case_key_exchange_rsp_setup_version_any},
    {SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_INVALID_REQUEST, "spdm_test_case_key_exchange_rsp_invalid_request", spdm_test_case_key_exchange_rsp_invalid_request, spdm_test_case_key_exchange_rsp_setup_version_any},
    {SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_SUCCESS_12, "spdm_test_case_key_exchange_rsp_success_12", spdm_test_case_key_exchange_rsp_success_12, spdm_test_case_key_exchange_rsp_setup_version_12},
    {SPDM_RESPONDER_TEST_CASE_KEY_EXCHANGE_RSP_SUCCESS_12_HS_CLEAR, "spdm_test_case_key_exchange_rsp_success_12_hs_clear", spdm_test_case_key_exchange_rsp_success_12_hs_clear, spdm_test_case_key_exchange_rsp_setup_version_12_hs_clear},
