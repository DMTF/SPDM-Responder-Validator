...
Description: SPDM responder shall return valid END_SESSION_ACK, if it receives a END_SESSION in PSK session.
...
Description: SPDM responder shall return ERROR or ignore, if it receives a END_SESSION after session terminated in DHE session.
...
9. Requester -> PSK_EXCHANGE {SPDMVersion=NegotiatedVersion, ...}
10. PSK_EXCHANGE_RSP <- Responder
11. Requester -> PSK_FINISH {SPDMVersion=NegotiatedVersion, ...}
12. PSK_FINISH_RSP <- Responder
13. Requester -> END_SESSION {SPDMVersion=NegotiatedVersion, ...} in session-X
14. END_SESSION_ACK <- Responder in session-X
...
Description: SPDM responder shall return ERROR(VersionMismatch), if it receives a END_SESSION with non negotiated version in PSK session.
...
Description: SPDM responder shall return ERROR(UnexpectedRequest), if it receives a END_SESSION in PSK session handshake.
