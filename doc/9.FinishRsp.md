...
Description: SPDM responder shall return valid FINISH_RSP without MULTI_KEY_CONN_RSP, if it receives a FINISH with negotiated version 1.2+.
...
Assertion 9.9.1:
    sizeof(SpdmMessage) >= sizeof(FINISH_RSP)
    sizeof(SpdmMessage) >= sizeof(FINISH_RSP) + sizeof(SpdmMessage.OpaqueDataLength) version 1.4+
...
Description: SPDM responder shall return valid FINISH_RSP with HANDSHAKE_IN_THE_CLEAR without MULTI_KEY_CONN_RSP, if it receives a FINISH with HANDSHAKE_IN_THE_CLEAR_CAP and negotiated version 1.2+.
...
Assertion 9.10.4:
    HMACVerify (finished_key, SpdmMessage.ResponderVerifyData, TH.HMAC.FINISH_RSP)
...
Description: SPDM responder shall return valid FINISH_RSP with MULTI_KEY_CONN_RSP, if it receives a FINISH with negotiated version 1.3+.
...
Description: SPDM responder shall return valid FINISH_RSP with HANDSHAKE_IN_THE_CLEAR with MULTI_KEY_CONN_RSP, if it receives a FINISH with HANDSHAKE_IN_THE_CLEAR_CAP and negotiated version 1.3+.
